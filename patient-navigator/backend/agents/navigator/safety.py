...
1. A fixed keyword/pattern check runs against the patient's raw message,
   independent of anything the LLM said.
2. If either the deterministic check OR the agent's own output indicates
   a possible emergency, the result is forced to the escalation path —
   this is a one-way, monotonic override (safety can escalate a normal
   result but nothing here ever downgrades an emergency the LLM flagged).
...
Phase 3 will replace this with a stronger, deterministic triage system;
...
# false negatives could mean a missed emergency, so this errs wide.
_EMERGENCY_PATTERNS = [
    r"\bchest pain\b",
...
def matches_emergency_pattern(patient_message: str) -> bool:
    return bool(_EMERGENCY_REGEX.search(patient_message))
