"""
Django settings for the Patient Navigator project (Phase 1 — Foundation).
...
# Database
# ---------------------------------------------------------------------------
# DATABASE_URL example: postgres://user:password@localhost:5432/patient_navigator
# Falls back to a local sqlite file only when DATABASE_URL is not set, which
# keeps `manage.py test` / local scaffolding runnable without Postgres.
DATABASES = {
...
REST_FRAMEWORK = {
    ...
    "DEFAULT_THROTTLE_RATES": {
        # Deliberately tight on anonymous auth endpoints (register/login) to
...
# ---------------------------------------------------------------------------
# Logging — never log request/response bodies (patient messages live there).
# ---------------------------------------------------------------------------
LOGGING = {
    "version": 1,
    ...
    "loggers": {
        ...
        # Explicitly kept quiet: request bodies can contain patient messages.
...
# ---------------------------------------------------------------------------
# Security hardening (relaxed automatically in DEBUG for local dev)
# ---------------------------------------------------------------------------
if not DEBUG:
